...
/**
 * Handled in Server Actions to transform internal errors into user-facing messages.
 */
export function handleMutationError(error: ActionError) {
  const { reason, message, errors } = error;
  ...
  switch (reason) {
    ...
    case "Failed to create equipment":
      return {
    ...
    default:
      // If you get a red squiggle here, a member of ActionErrorReason is missing a 'case' above.
